...
for h, d in zip(human_files_short, dog_files_short):
    human_percent = human_percent + face_detector(h)
    dog_percent = dog_percent + face_detector(d)
    
print('human_files 的前100张图像中，能够检测到人脸的图像占比为{}%。'.format(human_percent),
      '\n',
       'dog_files 的前100张图像中，能够检测到人脸的图像占比为{}%。'.format(dog_percent))
...
# 
# __回答:__对于基于面部识别的身份认证这样的应用是合理的。但对于普通应用不是很合理，可以使用数据生成器生成一些旋转，平移过后的增强图片，对模型进行训练。或者增加池化层，使模型能够更好地泛化。
# 
# 
...
for h, d in zip(human_files_short, dog_files_short):
    human_percent = human_percent + dog_detector(h)
    dog_percent = dog_percent + dog_detector(d)
    
print('human_files_short中图像检测到狗狗的百分比为{}%。'.format(human_percent),
      '\n',
       'dog_files_short中图像检测到狗狗的百分比为{}%。'.format(dog_percent))
...
# __回答:__   
# 问题一：根据上图提示搭建。  
# 问题二：如上的架构能够在该问题上取得很好的表现，因为：  
# 1.Conv2D_1使用16个过滤器，窗口大小为(2,2)，步长默认(1,1)，padding设置为same，意味着用0填充节点，使用relu函数，该函数意味着小于0时为0，大于0时等于本身，导数为1，可在不牺牲精度的前提下极大提升训练，参数数量=K\*F\*F\*D_in + K=208，该层使输入与权重相乘以此增加深度而保持数据的宽和高不变，该层对线条和色块进行过滤。  
# 2.MaxPooling2D_1窗口为(2,2)，步长(2,2),取2\*2窗口内最大值作为新的节点，意味着原本4个节点，现在使用4个节点中的最大值进行表示，不改变深度的同时减小数据的宽和高，减少参数，避免过拟合。  
# 3.Conv2D_2使用32个过滤器，窗口大小为(2,2)，步长默认(1,1)，padding设置为same，意味着用0填充节点，使用relu函数，该函数意味着小于0时为0，大于0时等于本身，导数为1，可在不牺牲精度的前提下极大提升训练，参数数量=K\*F\*F\*D_in + K=2080，该层使输入与权重相乘以此增加深度而保持数据的宽和高不变，该层对更复杂的圆圈或条纹进行过滤。  
# 4.MaxPooling2D_2窗口为(2,2)，步长(2,2),取2\*2窗口内最大值作为新的节点，意味着原本4个节点，现在使用4个节点中的最大值进行表示，不改变深度的同时减小数据的宽和高，减少参数，避免过拟合。  
# 5.Conv2D_3使用64个过滤器，窗口大小为(2,2)，步长默认(1,1)，padding设置为same，意味着用0填充节点，使用relu函数，该函数意味着小于0时为0，大于0时等于本身，导数为1，可在不牺牲精度的前提下极大提升训练，参数数量=K\*F\*F\*D_in + K=8256，该层使输入与权重相乘以此增加深度而保持数据的宽和高不变，该层对更复杂的特征组合例如眼睛，嘴巴等进行过滤。   
# 6.MaxPooling2D_3窗口为(2,2)，步长(2,2),取2\*2窗口内最大值作为新的节点，意味着原本4个节点，现在使用4个节点中的最大值进行表示，不改变深度的同时减小数据的宽和高，减少参数，避免过拟合。  
# 7.GlobalAveragePooling2D将每个特征映射求平均值，每个特征映射都缩减成一个值，原本的三维数组此时变成一个向量，而Dense层无法处理三维数据，所以必须在这里加上一个GlobalAveragePooling2D使其变成二维数据。  
# 8.Dense使用上一层输入的二维数据，输出133个分类的得分，softmax函数 = 每一类得分 / 全部类总得分，意味着可以计算出属于每一类的概率，概率最大的类别即是预测的结果。
...
# 
# __回答:__   
# 问题一：使用VGG19最后一层作为输入；使用了数个卷积层继续从数据中找特征；每个卷积层分配一个dropout避免过拟合，提高准确率；使用数个池化层降维，最后使用一个平均池化层将三维数据变为二维，一个卷积层输出结果。  
# 问题二：使用该网络架构的原因是，VGG19相对于Resnet50和Xception作为输入结果更差，用更差的输入，是想看一下如何在输入数据并不好的情况下，尽可能得到更好地结果。使用Resnet50和Xception的话，可以轻易达到80%的准确率。  
#   
# 问题三：什么这一架构会在分类任务中成功：这些现有架构是从无数个架构和大量的调试超参数的成果，它们已经在庞大的ImageNet数据库上进行了长期训练，这一架构已经学会区分ImageNet中存在的1000种不同类别的图片，其中包括动物，日常可见的物品等。我们知道卷积神经网络的第一层卷积神经一般是检测边缘和色块，第二层则是圆圈，条纹，长方形等等，第三层则可能会有眼睛，鼻子或者鸟，汽车。那么对于我们所要区分的对象：“狗的品种”，我们可以利用这些已经训练好的过滤出来的特征对图片进行识别，而不用再花费几个月的时间去重新调整超参数和结构，然后再花几周的时间进行训练，因为边缘色块，圆圈条纹，眼睛鼻子已经存在于这一构架当中，所以预训练模型直接替我们省去了这一步骤。迁移学习的特别之处只需要将先前的权重冻结，然后将最后的几个层级替换即可使用。  
# 为什么早期的尝试不如这一次成功：1.早期的尝试训练次数少。2.早期的尝试深度较浅。3.早期的尝试没有预先过滤出的边缘色块，圆圈条纹，鼻子眼睛等特征。4.早期的尝试并没有对超参数和结构进行优化。
# 
...
VGG19_model.add(Conv2D(filters=16, kernel_size=1, padding='same', activation='relu', input_shape=train_VGG19.shape[1:]))
VGG19_model.add(Dropout(0.1, seed=64))
VGG19_model.add(Conv2D(filters=32, kernel_size=1, padding='same', activation='relu'))
VGG19_model.add(Dropout(0.1, seed=64))
VGG19_model.add(Conv2D(filters=64, kernel_size=2, padding='same', activation='relu'))
VGG19_model.add(Dropout(0.1, seed=64))
VGG19_model.add(MaxPooling2D(pool_size=2))
VGG19_model.add(Conv2D(filters=128, kernel_size=2, padding='same', activation='relu'))
VGG19_model.add(Dropout(0.1, seed=64))
VGG19_model.add(MaxPooling2D(pool_size=2))
...
def whatsdog(img_path):
    if dog_detector(img_path):
        to_show_image = mpimg.imread(img_path)
        plt.imshow(to_show_image)
        plt.show()
        print('It\'s a {}!'.format(VGG19_predict_breed(img_path)[7:].replace('_', ' ')))
        ...
        plt.imshow(to_show_image)
        plt.show()
        print('You look like a {}!'.format(VGG19_predict_breed(img_path)[7:].replace('_', ' ')))
        return VGG19_predict_breed(img_path)
    else:
        to_show_image = mpimg.imread(img_path)
        plt.imshow(to_show_image)
        plt.show()
        print('It\'s a wrong picture,啥也不是，换一张吧。')
...
## 自由地使用所需的代码单元数吧
pictures = np.arange(6)+1
for i in pictures:
    print(i)
    whatsdog(str(i)+'.jpg')
print('All pictures are from the Internet.')


# 问题一：输出结果和我预期的一样，准确率没有很高。人对应的狗还是比较像的，但对于美国爱斯基摩犬和萨摩耶模型并没能分辨出来。  
# 问题二：  
# 1.进行数据增强。  
# 2.将特征向量从VGG19中提取替换为从Resnet50或Xception中提取。  
# 3.尝试其他的参数组合。  
# 4.自行搭建新模型。  
